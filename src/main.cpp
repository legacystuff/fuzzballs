...
#if defined(NDEBUG)
# error "FuzzCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xefc1b966666544e7b75f17836c8af5b67545721c2a97bb08975b72faa9e4aabf");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FuzzCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FuzzCoin Signed Message:\n";
...
{
    // FuzzCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FuzzCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FuzzCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x07d3b3dfacbdb8caaaa7330e681714e73adaea010e3f9ad5283bda012424c3b0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "September 25, 2015 at 03:06 BST Bitcoin Processor BitPay Reduces Staff in Cost-Cutting Effort";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1443335038;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 109825460;
        ...
        assert(block.hashMerkleRoot == uint256("0xdd7b65b01262e5733e1479913e0a74934a5775ef32f56e2362d7f9b366ea90f7"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // FuzzCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FuzzCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FuzzCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FuzzCoinMiner(CWallet *pwallet)
{
    printf("FuzzCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("FuzzCoin-miner");
    ...
    try { loop {
        ...
        printf("Running FuzzCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FuzzCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FuzzCoinMiner, pwallet));
}
